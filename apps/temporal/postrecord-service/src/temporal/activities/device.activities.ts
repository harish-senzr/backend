...
export async function prepareData(payload: any) {
  try {
    ...
    const controllerRes = await axios.get(controllerUrl, { headers });
    ...
    const userRes = await axios.get(`${BASE_URL}/items/personalModule`, {
      headers,
      params: {
        filter: {
          employeeId: {
    ...
    const assignedUserRes = await axios.get(`${BASE_URL}/users/${personalModule.assignedUser}`, { headers });
    ...
    const response = await axios.post(`${BASE_URL}/items/logs`, finalPayload, {
      headers: {
...
export async function validateMode(payload: any) {
  try {
...
export async function getAttendanceRecord(employeeId: number | string, date: string, tenant: string) {
  try {
...
export async function checkIfEmpty(data: any) {
...
export async function getShiftConfigByEmployeeId(employeeId: string) {
  try {
...
export async function getMondayShiftDetails(data: any) {
  try {
    const monJShifts: string[] = data?.attendanceSettings?.monJ?.shifts || [];
    ...
    const shiftDetails = await Promise.all(
      monJShifts.map(async (shiftId: string) => {
        try {
...
export async function calculateLateBy(shiftEntryTime: string, punchTime: string, date: string): Promise<string | null> {
...
export function calculateWorkHours(inTime: string, outTime: string): string | null {
  try {
...
export function calculateEarlyDeparture(scheduledExitTime: string, actualOutTime: string): string | null {
  try {
    if (!scheduledExitTime || !actualOutTime) return null;
...
export async function insertAttendance(payload: any): Promise<any> {
  try {
    ...
    if (existing) {
      ...
      const updateResponse = await axios.patch(updateUrl, payload, {
        headers: {
          Authorization: `Bearer ${DIRECTUS_TOKEN}`,
        },
      });
      ...
      const insertResponse = await axios.post(`${BASE_URL}/items/attendance`, payload, {
        headers: {
          Authorization: `Bearer ${DIRECTUS_TOKEN}`,
        },
      });
